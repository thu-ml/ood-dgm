...
def get_mutated_dataloader(mut_rate):
    def ret(*args, **kw):
        dl = DataLoader(*args, **kw)
        # mutate dl.data
        mask = (dl.rng.rand(*dl.data.shape) < mut_rate).astype('f')
        noise = dl.rng.randint(0, 256, dl.data.shape)
        dl.data = (1-mask) * dl.data + mask * noise
        return dl
    return ret
...
class DataLoader(object):
    ...
    def __init__(self, data_dir, subset, batch_size, rng=None, shuffle=False, return_labels=False,
        ...
        assert len(self.labels.shape) == 1 and self.data.dtype == np.uint8
