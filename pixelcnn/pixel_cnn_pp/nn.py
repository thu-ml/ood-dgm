...
def discretized_mix_logistic_loss(x,l,sum_all=True,fake_3channels=False):
    ...
    if fake_3channels:
        log_probs = log_probs[:,:,:,0] + log_prob_from_logits(logit_probs)
    else:
...
def sample_from_discretized_mix_logistic(l,nr_mix,fake_3channels=False):
    ls = int_shape(l)
    xs = ls[:-1] + [3]
    # unpack parameters
    logit_probs = l[:, :, :, :nr_mix]
