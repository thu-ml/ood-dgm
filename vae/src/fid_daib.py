...
def imread(f):
    return scipy.misc.imread(f, mode='RGB')
...
def get_activations_tf(images, sess, batch_size=100, verbose=False):
...
def check_or_download_inception():
    ...
    model_file = os.path.expanduser('~/inception/classify_image_graph_def.pb')
...
def evaluate_fid_score(fake_images, dataset, root_folder, norm=True):
    ...
    real_out = get_activations_tf(real_images, sess, verbose=True)
    fake_out = get_activations_tf(fake_images, sess, verbose=True)
